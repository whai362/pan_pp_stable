...
def evaluate_method(evaluationParams):
    ...
    def load_ctw_gt(gt_root):
        def get_ann(gt_path):
            ...
            for line in lines:
                ...
                bbox = [np.int(gt[i]) for i in range(28)]
                ...
                ann.append((bbox, word))

            return ann

        gt_names = [gt_name for gt_name in mmcv.utils.scandir(gt_root, '.txt')]
        gt = {}
    ...
    # gt = rrc_evaluation_funcs.load_zip_file(gtFilePath,evaluationParams['GT_SAMPLE_NAME_2_ID'])
    # subm = rrc_evaluation_funcs.load_zip_file(submFilePath,evaluationParams['DET_SAMPLE_NAME_2_ID'],True)
    gt = load_ctw_gt('../../data/CTW1500v2/ctw1500_e2e_test/')
    subm = load_pred('../../outputs/submit_ctw_rec/')
